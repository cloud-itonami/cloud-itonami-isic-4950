(ns pipeline.operation
  "OperationActor -- one pipeline-transport operation = one supervised
  actor run, expressed as a langgraph-clj StateGraph. The advisor
  (PipelineTransport advisor) is sealed into a single node (:advise);
  its proposal is ALWAYS routed through the Pipeline Integrity Governor
  ...
  One graph run = one pipeline operation (intake -> advise -> govern ->
  decide -> commit | hold | approval). No unbounded inner loop -- each
  operation is auditable and checkpointed. A batch's life is advanced
  by MANY ops (verify / dispatch / settle), each its own independent
  run.

  Human-in-the-loop = real approval workflow:
  `interrupt-before #{:request-approval}` pauses the actor and hands the
  decision to a human operator (pipeline controller / terminal
  superintendent). The approver resumes with `{:approval {:status
  :approved}}` (or :rejected). `:batch/dispatch`/`:delivery/settle`
  ALWAYS reach this node when the governor is clean -- see
  `pipeline.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [pipeline.pipelineadvisor :as pipelineadvisor]
...
(defn build
  ...
  opts:
    :advisor      -- a `pipeline.pipelineadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (pipelineadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; PipelineTransport advisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (pipelineadvisor/-advise advisor store request)]
            {:proposal p :audit [(pipelineadvisor/trace request p)]})))

      ;; Pipeline Integrity Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
